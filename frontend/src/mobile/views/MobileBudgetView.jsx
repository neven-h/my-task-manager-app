...
import { TrendingUp, TrendingDown, Scale, Plus, Upload } from 'lucide-react';
        ...
        <div style={{ minHeight: '100vh', background: IOS.bg, fontFamily: FONT_STACK, paddingBottom: selectMode ? 'calc(env(safe-area-inset-bottom, 0px) + 140px)' : 'calc(env(safe-area-inset-bottom, 0px) + 80px)' }}>
            ...
            {activeTabId !== null && (
                <MobileBudgetClearStrip tabEntries={tabEntries} loading={loading}
                    confirmClearTab={confirmClearTab} setConfirmClearTab={setConfirmClearTab}
            ...
            {activeTabId && (
                <div style={{ padding: '0 16px', marginBottom: 4 }}>
                    <button onClick={() => setShowUpload(true)} style={{
                        ...
                        fontWeight: 600, fontSize: '0.85rem', color: IOS.blue, fontFamily: FONT_STACK,
                    }}>
                        <Upload size={16} /> Upload Budget File
                    </button>
                </div>
            )}
            ...
            <div style={{ padding: '16px 16px 0' }}>
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: activeTabId ? 12 : 16 }}>
                    <button onClick={() => openAdd('income')} style={{ flex: 1, padding: '12px 0', border: 'none', borderRadius: 12, background: IOS.green, color: '#fff', fontWeight: 600, fontSize: '0.9rem', cursor: 'pointer', fontFamily: FONT_STACK, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                        <Plus size={16} /> Income
                    </button>
                    <button onClick={() => openAdd('outcome')} style={{ flex: 1, padding: '12px 0', border: 'none', borderRadius: 12, background: IOS.red, color: '#fff', fontWeight: 600, fontSize: '0.9rem', cursor: 'pointer', fontFamily: FONT_STACK, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
