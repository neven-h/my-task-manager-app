    ...
    return (
        <div style={{ background: SYS.bg, border: `3px solid ${SYS.border}`, overflow: 'hidden' }}>
            ...
            <div style={{
                ...
                <div style={{ flex: 1 }} />
                {selectMode && visibleEntries.length > 0 && (
                    <button type="button" onClick={() => onSelectAll(visibleEntries.map(e => e.id))}
                        style={{ ...filterBtn(false), fontSize: '0.72rem', color: SYS.primary, borderColor: SYS.primary }}>
                        {selectedIds?.size === visibleEntries.length ? '☑ Deselect All' : `☐ Select All (${visibleEntries.length})`}
                    </button>
                )}
